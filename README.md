...
자동 검증(라우팅 정확도, 트레이스 완결성 등) AND Judge LLM 점수, 두 조건 모두 통과해야 합격이라 개별 모듈보다 합격률이 낮습니다. 동일한 28개 케이스로 v1/v2를 비교했습니다. Safety·Expert 평가는 두 버전이 동일 에이전트를 공유하므로 v1 결과를 공통 적용합니다.

| 항목 | v1 (라우터) | v2 (Tool Calling) |
|---|---|---|
| **E2E 합격률** | **60.7% (17/28)** | 50.0% (14/28) |
| 평균 레이턴시 | 5,640ms | 6,015ms |
| 평균 LLM 호출 수 | 2.9회 | 3.2회 |
| Fallback 발생률 | 10.7% | 10.7% |
| 복합 문의 처리율 | — | 17.9% |

v3는 도메인 특화 FAQ 데이터를 인덱싱한 환경에서 완결성 개선 효과를 측정할 수 있으며, 일반 테스트셋으로는 v2 대비 의미 있는 수치 비교가 어렵습니다.
...
v1이 v2보다 E2E 합격률이 높은 주된 이유는 **카테고리 정확도** 차이입니다. v2는 복합 문의를 `multi`로 분류하지만 테스트셋의 expected_category가 단일 카테고리로 정의되어 있어 자동 검증에서 오분류로 판정됩니다. 실제 복합 문의 처리 품질(Judge R/C/S)은 v2가 동등하거나 높습니다.
